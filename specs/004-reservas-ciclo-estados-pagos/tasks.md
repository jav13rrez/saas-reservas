...
- [x] T001 [US1] Add `completed` and `no_show` to `BookingStatus` and `TRANSITIONS`
...
- [x] T002 [US3] New `packages/domain/src/payments/manual-payment.ts`: `ManualPaymentMethod`,
...
- [x] T003 [US1] `BookingService.complete()` + `noShow()` in
  `services/api/src/application/bookings/booking-service.ts` (transition + event + audit), mirroring
  approve/reject/cancel.
- [x] T004 [US1] Admin routes `POST /v1/admin/bookings/:id/{approve,reject,complete,no-show}` in
...
- [x] T005 [US1] Notification dispatch: add `completed`/`no_show` message builders to
  `services/worker/src/jobs/booking-notification-dispatcher.ts`.
- [x] T006 [US1] Integration test `tests/integration/bookings/lifecycle.test.ts`: complete/no-show
  audit; reject frees occupancy; invalid transition rejected.

## Phase 3: US2 — Configurable default status (P2)

- [x] T007 [US2] Thread `requiresApproval` into `AdminBookingService.createBooking`: when true, stop
...
- [x] T008 [US2] E2E test `tests/e2e/admin-bookings-lifecycle.test.ts`: with requiresApproval=true →
  create yields Pending; approve → Approved; reject → Rejected + slot freed; with false → Approved.

## Phase 4: US3 — Manual payments (P3)

- [x] T009 [US3] Migration `infra/postgres/012-manual-payments.sql` (RLS) + `manualPayments` in
  `schema.ts`.
- [x] T010 [US3] `ManualPaymentRepository` port + in-memory + Drizzle adapters; `ManualPaymentService`
  (`upsertForBooking` audited, `getForBooking`).
- [x] T011 [US3] Routes `GET`/`PUT /v1/admin/bookings/:id/payment` (admin-gated). Integration test
...
- [x] T014 [P] ADR-0024 (booking lifecycle states + manual-payment model).
- [x] T015 [P] Quickstart + acceptance; mark tasks.md complete.
- [x] T016 Full gate green (typecheck/lint/format/test + apps build); update PROGRESS/HANDOFF/TECH_DEBT
