...
4. **Real adapter wiring**: swap fake gateway, message provider, KMS, and storage adapters for Stripe Connect, SendGrid/Twilio, AWS KMS, and S3. **Stripe Connect — DONE (2026-06-19, ADR-0019)**: `StripePaymentGateway` + `FetchStripeHttp` behind the existing port, destination charges + application fee, selected by `STRIPE_SECRET_KEY` (fake stays default). Remaining in this item: SendGrid/Twilio messaging, AWS KMS, S3 — plus the Stripe follow-ups in `TECH_DEBT.md` (DB-backed vault for connected-account ids, checkout payment-method/webhook-capture flow, webhook signature verification). **Next session starts here**, after the live `api`-mode validation below.
