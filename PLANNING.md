...
1. **Canonical domain/persistence hub migration** — *DONE (2026-06-17)*. Additive layer (join tables `resource_services`/`resource_locations`/`resource_employees`, `004-resource-hub.sql`, domain helpers, `ResourceHubRepository`/`ResourceHubService`) plus the **read-model cutover**: `AvailabilityService`, checkout, reschedule, and Fastify admin now read the hub (interchangeable resource pool collapsed into one synthetic engine demand; see `hub-resources.ts`). Legacy model-B tables/ports are retained but no longer drive the read path. **Remaining:** add provider→locations canonically (hub location compatibility is currently a no-op since the canonical `Provider` has no locations); then a destructive migration may drop `provider_resources` + `service_resources`.
